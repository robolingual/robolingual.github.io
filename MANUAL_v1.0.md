...
* 範囲は ⅓×〜3.0× です。
* つまみを狙う必要はありません。スライダーの周りのどこを触っても掴めます。

    * 触った場所へ飛ぶことはなく、そこを起点に動かした量だけ変わります

* 目盛りの数字は、触れた瞬間にその速度へ飛びます。⅓× 0.5× 1.0× 2.0× 3.0× のどれでも同じです。
* ダブルクリックでも 1.0× に戻ります。
* 操作盤はそれぞれ独立しています。スライダーを掴んだまま、別の指で16パッドを押したり八卦盤を動かしたりできます。
...
* 押した瞬間から遡って1拍ぶんを切り取ります。押している間は同じ素材を使い続けます。
* 1 を押せば、今さっき鳴っていた1拍がそのまま返ります。頭は音の立ち上がりに合わせているので、アタックがループの先頭に来ます。
* 波形は2段×3列の6区画に分かれています。押さえたまま指をずらすと刻みが変わります。

    * 上段：左=1、中=1/32、右=1/16
    * 下段：左=1/2、中=1/4、右=1/8

* 数字は1拍を何分割するかです。1 が1拍まるごと、1/2 がその半分、と細かくなっていきます。
* 刻みを変えても音の出頭は動きません。ループの終わりだけが伸び縮みします。
* 1/16、1/32 まで刻むと繰り返しが速すぎて、音程のついたブザーのように聴こえます。刻みとして使えるのは 1/8 あたりまでで、その先は別の音色だと思ってください。
* サンプリングしている範囲は、波形の上に白く光る帯で表示されます。刻みが細かいほど点滅が速くなり、目で追えない速さになると何回かに1回だけ光って見やすさを保ちます。
...
* 機種ごとの設定表は要りません。その場で触って覚えさせる方式です。

    * MIDI を押すと、覚えさせる画面が出ます
    * 画面に「NINJA A」のように出るので、それに当てたいパッドを叩きます
    * 16パッドが終わるとPANの番です。当てたいノブを回します
    * 計20個で完了です。進み具合は「7 / 20」のように出ます
    * とばす：今の1個を割り当てずに次へ。パッドが16個ない機種で使います
    * やめる：そこまでの割り当てを残して終了します

* 割り当てはブラウザに保存されます。次に開いたときは覚えたままです。
* やり直したいときは、もう一度 MIDI を押してください。前の割り当ては消えて、最初から取り直しになります。
* 八卦盤とスリップループはMIDIに割り当てていません。どちらも位置を面で指し続ける操作で、パッドやノブでは置き換えられないためです。
...
* 選んだ画像はブラウザに保存されます。次に開いたときもそのままです。
* 大きすぎる画像は自動で縮めてから保存します。元のファイルには手を付けません。
