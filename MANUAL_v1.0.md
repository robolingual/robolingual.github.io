...
* 各キャラクターに4つのバリエーションがあります。
    ...
    * A：短く、密で、乾いた発音
    * B：長く、丸く、アタック少なめ（BANKER の B だけはスラップベースのソロです）
    * C：4スライスの固定パターン
    * Ex：ワンショットの必殺技
...
* ◀REV を押すと、ループが丸ごと逆から流れます。もう一度押すと元に戻ります。ボタンは八卦盤の左、0 LOCK の下にあります。

    * 頭出しではありません。押した場所からそのまま後ろへ巻き戻るので、フレーズの途中でも使えます
    * 4段もサウンドロゴも一緒に裏返ります。頭はずれません
...
* ノブはドラッグして回します。
* ノブの真下、色付きの帯に NINJA などの名前が書かれている部分を左右になぞっても、同じPANが動きます。ノブは小さく、触ると指で隠れて向きが見えなくなるためです。

    * なぞれるのは名前が出ている左端だけです。帯のうちパッドが乗っている部分は、パッドの操作が優先されます
...
* 盤には途中に境目があります。中心から外周までの3分の1あたりです。

    * そこまではロボ語にエフェクトが深くかかっていくだけです
    * そこを越えると、読み込んだ音源そのものが引き伸ばされて鳴りはじめ、外周ではそれだけになります
    * 目印は描かれていません。一度は縁まで引いてみてください
...
* 押さえたまま、もう一方の手で八卦盤を動かせます。切り取ったループにそのままエフェクトが乗ります。
* 指を離すと元の演奏に戻ります。
